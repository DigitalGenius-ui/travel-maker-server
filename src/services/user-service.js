...
export const updateImage = async ({ userImg, userId }) => {
	...
	AppAssert(updateProfileImg, CONFLICT, "Failed to update image in the database!");
...
export const getSingleMomentPost = async id => {
	...
	AppAssert(singleMoment, INTERNAL_SERVER_ERROR, "Failed to get single moment!");
	return { singleMoment };
};

// get all moment post
export const getAllMomentPosts = async () => {
...
export const createMomentComment = async (data, userId) => {
	...
	AppAssert(createComment, INTERNAL_SERVER_ERROR, "Failed to create moment comment!");
...
export const changeUserPassword = async (data, id) => {
	...
	const isPasswordMatching = await comparePass(currentPassword, findUser.password);
